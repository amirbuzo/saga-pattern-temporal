package com.ikubinfo.orchestrator.infrastructure.temporal.workflow.impl;
...
public class IADLFulfillmentWorkflowImpl implements IADLFulfillmentWorkflow {
	...
	private final ActivityOptions buildActivityOptions = ActivityOptions
			.newBuilder().setStartToCloseTimeout(Duration.ofMinutes(1))
			.setTaskQueue(TaskQueue.BUILDING_ACTIVITY_TASK_QUEUE.name())
			.setRetryOptions(
					RetryOptions.newBuilder().setMaximumAttempts(3).build())
			.build();

	private final ActivityOptions jiraActivityOptions = ActivityOptions
			.newBuilder().setStartToCloseTimeout(Duration.ofMinutes(1))
			.setTaskQueue(TaskQueue.JIRA_ACTIVITY_TASK_QUEUE.name())
			.setRetryOptions(
					RetryOptions.newBuilder().setMaximumAttempts(3).build())
			.build();
	private final ActivityOptions sonarActivityOptions = ActivityOptions
			.newBuilder().setStartToCloseTimeout(Duration.ofMinutes(1))
			.setTaskQueue(TaskQueue.SONAR_ACTIVITY_TASK_QUEUE.name())
			.setRetryOptions(
					RetryOptions.newBuilder().setMaximumAttempts(3).build())
			.build();
	...
	// .build();
	private final ActivityOptions taskActivityOptions = ActivityOptions
			.newBuilder().setStartToCloseTimeout(Duration.ofMinutes(1))
			.setTaskQueue(TaskQueue.TASK_ACTIVITY_TASK_QUEUE.name())
			.setRetryOptions(
					RetryOptions.newBuilder().setMaximumAttempts(10).build())
			.build();
 
	private final BitbucketActivities bitBucketActivities = Workflow
			.newActivityStub(BitbucketActivities.class, jiraActivityOptions);

	private final SonarActivities sonarActivities = Workflow
			.newActivityStub(SonarActivities.class, sonarActivityOptions);

	private final BuildActivities buildActivities = Workflow
			.newActivityStub(BuildActivities.class, buildActivityOptions);
	
	private final JiraTaskActivities taskActivities = Workflow
			.newActivityStub(JiraTaskActivities.class, taskActivityOptions);
