...
public class PrWorker {
    ...
    public void createWorker() {
        ...
        WorkflowImplementationOptions workflowImplementationOptions = WorkflowImplementationOptions.newBuilder().setFailWorkflowExceptionTypes(NullPointerException.class).build();
        
        var workerFactory = WorkerFactory.newInstance(workflowClient);
        var worker = workerFactory.newWorker(TaskQueue.IADL_FULFILLMENT_WORKFLOW_TASK_QUEUE.name(), workerOptions);
        
        var workerCompleteprRequest = workerFactory.newWorker(TaskQueue.TASK_ACTIVITY_TASK_QUEUE.name(), workerOptions);
        
        // Can be called multiple times
        worker.registerWorkflowImplementationTypes(workflowImplementationOptions, IADLFulfillmentWorkflowImpl.class);
