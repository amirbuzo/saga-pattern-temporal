...
public class SonarRepositoryImpl implements SonarRepository {
    ...
    public Optional<SonarResultDto> getByprRequestId(String prRequestId) {
        return buildMongoRepository.findByprRequestId(prRequestId).stream().map(SonarPersistableMapper.MAPPER::map).findFirst();
    }
    
    @Override
    public Optional<SonarResultDto> getByBranchId(String prRequestId) {
        return buildMongoRepository.findByBranchId(prRequestId).stream().map(SonarPersistableMapper.MAPPER::map).findFirst();
    }
}
